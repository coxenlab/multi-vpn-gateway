...
3. **配置使用热加载**:`manager.rebuild()` 以一致数据库快照构造候选，通过 `PUT {CTRL}/configs?force=true` 的 `payload` 应用，读回匹配后才保存启动文件；规则变更不主动重启 mihomo。同一有效内容和运行态均匹配时免重载/免写盘。保持既有连接是回归要求，不能仅凭一次 2xx 推断所有协议与并发故障下绝不断连。
   ...
   Python 的规则新增、单条/批量启停和备份导入只在异步层读取请求正文，写库、备份解析和入口同步交给框架工作线程；慢同步不会占住事件循环。上述四类操作的规划与写入仍在进程内串行，保留同名导入与重复规则检查的顺序。该锁不替代 SQLite 事务或跨进程配置投递锁，也不承诺多进程并发导入的全局去重。已开始的工作不因客户端断开而重放；结果不明应先读取当前配置核对。
   ...
   未创建实例且无恢复记录的通道可在休眠时停用/删除，规则同步仍标 pending。已有实例或恢复记录的删除默认返回 `409 / runtime_required` 并保留数据；确认框说明需要启动环境，用户确认后通过 `DELETE ?prepare_runtime=true` 显式准备，再按既有流程核实清理。目标已不存在不启动环境；启动恢复已完成同一删除时读回确认后返回成功。已有实例也可在休眠时停用：`channel_stop_intents` 持久化停止代次，规则退出有效集；返回 `202 / stop_pending:true`，页面显示“停用待确认”。Docker 恢复后在打开分流入口前核对完整实例 ID/正式名称并停止，读回确认为 stopped/不存在后才清除意图。命令应答不明只读回，身份不符或读回失败保留意图；探活与空闲释放均不能绕过该状态。Web 共用同一记录。实际启动环境后的停止/清理入口仍需运行验收。
...
原生备注草稿按通道在当前 App 会话中保留，切换页面不丢草稿；保存只确认提交时的文本，等待期间的新输入仍标未保存。双栈 `/note` 的 `expected_note` 在 SQLite IMMEDIATE 事务中核对与加密写入，冲突返回 409；Web 详情和向导自动留档也传原文。核对后的合并仍带最新原文，另一次并发修改仍会被拒绝。省略该字段保持旧客户端语义。原生版本列表忽略取消或过期响应，禁选不适用本机架构的版本；已有版本保留用于编辑其他资料。连接保存期间锁定表单，文本配置读取上限 1 MiB。
