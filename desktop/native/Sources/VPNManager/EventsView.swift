...
struct EventsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                if let enabled { Toggle("记录日志", isOn: Binding(get: { enabled }, set: { value in Task { await setEnabled(value) } })).toggleStyle(.switch).disabled(loading || model.busy.contains("__events")) }
                Button(paused ? "继续刷新" : "暂停刷新") { paused.toggle() }
            }
            HStack {
    ...
    private func load() async {
        ...
        do {
            let feed = try await api.get("/api/events?since_seq=\(cursor)&limit=1000", as: EventFeed.self)
    ...
    private func setEnabled(_ value: Bool) async {
        if await model.perform("/api/events/enabled", key: "__events", body: ["enabled": value], success: value ? "日志记录已开启" : "日志记录已关闭") { await load() }
    }
    private func export() async {
        guard let api = model.api, !model.busy.contains("__events-export") else { return }
        model.busy.insert("__events-export"); defer { model.busy.remove("__events-export") }
