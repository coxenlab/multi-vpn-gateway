...
struct ContainersView: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            HStack { Text("容器状态").font(.title2.bold()); Spacer(); Button("刷新") { Task { await load() } }.disabled(loading) }.padding()
    ...
    private func load() async {
        ...
        do { report = try await api.get("/api/containers", as: ContainerFeed.self); error = nil }
