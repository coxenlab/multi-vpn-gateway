...
//! vpnmgr 桌面壳(Tauri v2)。
//!
//! 启动:准备本地组件与配置 → 进程内起 axum → 打开管理界面；连接通道时才启动专属 VM。
...
use vpnmgr_core::{app, config::Config, infra};
...
fn prepare_runtime(handle: &tauri::AppHandle) -> anyhow::Result<()> {
    std::env::set_var("VPNMGR_MANAGED_VM", "1");
    if let Ok(resources) = handle.path().resource_dir() {
        std::env::set_var("VPNMGR_BUNDLED_IMAGES_DIR", resources.join("images"));
    }
...
/// 先开放管理界面；VM 由 core 在用户连接/创建/恢复时按需启动。
async fn boot(handle: &tauri::AppHandle) -> Result<(), BootFailure> {
    let mut log = StepLog::default();
    let reporter = BootReporter::new(handle);
    let started = boot_step_start(BootStep::Runtime);
    reporter.update(BootStep::Runtime, BootStatus::Active, "准备本地管理界面…");
    prepare_runtime(handle).map_err(|e| BootFailure::new(BootStep::Runtime, e, &log))?;
    log.push("运行组件与基础参数已就绪；连接通道时才启动底座");
    reporter.update(BootStep::Runtime, BootStatus::Done, "本地组件已就绪");
    boot_step_done(BootStep::Runtime, started, "done");
    let started = boot_step_start(BootStep::Service);
    reporter.update(BootStep::Service, BootStatus::Active, "读取配置…");
    let cfg = Config::load();
    let (listener, state) = app::bootstrap(cfg).await
        .map_err(|e| BootFailure::new(BootStep::Service, e, &log))?;
    let _ = SHUTDOWN_STATE.set(state.clone());
    let port = listener.local_addr()
        .map_err(|e| BootFailure::new(BootStep::Service, e, &log))?.port();
    tauri::async_runtime::spawn(async move {
        if let Err(e) = app::serve(listener, state).await {
            vpnmgr_core::ev!(error, "boot", "service_exited", "本地服务异常退出", { "error": e.to_string() });
        }
    });
    reporter.update(BootStep::Service, BootStatus::Done, "管理界面已就绪");
    if let Some(window) = handle.get_webview_window("main") {
        let url = format!("http://127.0.0.1:{port}/").parse()
            .map_err(|e| BootFailure::new(BootStep::Service, e, &log))?;
        window.navigate(url).map_err(|e| BootFailure::new(BootStep::Service, e, &log))?;
    }
    boot_step_done(BootStep::Service, started, "done");
    Ok(())
}
