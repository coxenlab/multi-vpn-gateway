# Changelog

本项目变更记录。格式参考 [Keep a Changelog](https://keepachangelog.com/zh-CN/1.1.0/)。

## [Unreleased]

## [1.3.1] - 2026-09-21

### Fixed / 修复
- EasyConnect / aTrust 及自装客户端桌面容器的 `/dev/shm` 由 Docker 默认 64M 提到 512M（manifest `shm_size_mb`，两栈同源）。客户端内嵌 Chromium 渲染较重的服务器登录页时共享内存越界触发 Bus error，表现为「一连接就跳回服务器地址页」、进不了账号密码页；换客户端版本无效。已有通道需重建容器生效。

### Changed / 变更
- 「登录备注」改为「备忘录」，只保存手动写入的内容；登录窗「键入到容器」不再自动把键入文本（可能含账号、密码）记入备注。已存内容不受影响，请自行清理其中的敏感信息。
- 通道详情新增「重建容器」（二次确认）：EasyConnect / aTrust / 无头类型用当前设置重建并需重新登录，自装客户端原地重启、保留已装客户端。
- 通道编辑新增「客户端版本」选择（EasyConnect 等有版本的类型），改版本会重建容器。

## [1.3.0] - 2026-09-15

### Fixed / 修复
- 桌面 VM 禁止 Colima 把来宾通配监听自动映射到宿主；回环 TCP、Docker Unix socket 及 app 自持的 loopback SSH 入口保留，下次 VM 启动生效。
- hagb 通道探活失败后可对照域名/IP 通路恢复 Dante 负 DNS 缓存；按容器代次限制恢复频率，成功仍以原内网 SOCKS 探活为准。
- 桌面版增加 VM 私网出站基础防护，覆盖本工具 VPN 网段的 IPv4 转发；启动提前下发、周期真实核对、原子更新失败保留旧规则。旧 usernet 的拨号槽耗尽是一类已定位故障，未把所有断链归为同因；VM OUTPUT、IPv6、公网黑洞及豁免路径仍需单独验证。
- 桌面版通道创建、修改、启停、删除脱离 HTTP 请求取消，并按通道串行；探活 generation 防止旧结果覆盖停止态。容器两阶段切换与完整失败回滚仍在推进。
- 两端现有通道的重建接入持久化替换：准备失败保留设置，GUI 等待登录时可在详情页恢复上一次设置与数据，已保存备注保留；真实探活成功后才清理旧资源。首次创建与旧容器丢失也走独立候选；原卷无人使用才只读复制，失败清除失效运行态并可重试，自装客户端缺失原实例会明确报错。

### Changed / 变更
- 桌面环境休眠时可保存已有通道的连接信息，原设置保留；页面区分“已保存”和“已应用”，支持继续编辑、撤销及主动应用并启动，保存不会唤醒环境。
- 尚未创建实例且无恢复记录的通道可离线停用或删除；已有实例的删除确认会说明需要启动环境，清理完成前保留通道和设置，失败可重试。
- 通道详情只显示链接指定的通道；链接失效或缺少通道 ID 时显示返回列表入口，隐藏通道操作与登录视图，避免误操作其他通道。
- 桌面运行环境关闭时仍可保存分流规则、开关与导入配置，显示“已保存，连接后生效”；显式“连接并同步”才启动环境。正常等待连接不显示故障警告，上次同步时间与本次待应用状态分开表达。
- 桌面先打开管理界面，连接、创建或恢复时才启动底座；并发请求共享启动，页面显示进度与失败，旧登录记录不冒充当前连通。空闲释放仍在接入。
- 桌面 runtime 改为固定来源/校验值构建，附匹配 guestagent 和文件清单；可显式构建 gvisor #698 的 v0.8.9 回移候选，默认仍为基线。候选默认 128 个在途连接、30 秒拨号上限，不代表无限容量或真实网络矩阵已通过。
- 双栈规则同步增加持久代次、运行态读回和重试入口；相同有效配置免重载，解析失败保留启动文件，部分完成明确展示。分流页区分保存的通道状态与临时掉线显示，避免把仍启用的规则算成已停用。
- 双栈共享探活与缓存状态，桌面复用低权限探针；页面隐藏暂停轮询，慢请求不重叠。手动检测保留新鲜 SOCKS 探活。
- 八个页面内联控制器移入 ES modules，统一主数据加载与失败反馈；aTrust 登录桌面固定尺寸并在浏览器缩放。
- 配置字段与加密配置原子提交；未改变的连接参数不重建目标容器。helper 0.2.0 区分已接受与已应用代次，校验 ACK、持久化和安装握手。
- 界面收束(对外越简单、对内越详细,1.3.0):导航收成 通道 / 分流规则 / 流量监控 / 设置 四屏;新建向导 5 步→3 步(创建即静默预检、登录后自动探活);通道详情去掉仿真登录页、重启按钮与重复的登录入口,域名与 IP 合成一张规则表;分流规则页去掉干线图;监控页去掉与总览重复的「通道测速」;设置页按「接入 / 环境 / 备份 / 运行日志」重组,接入方式只保留推荐路径、其余折叠;容器管理与常见问题降为设置内的入口。实现细节(docker run 预览、探活命令、no-resolve、卷 / MAC 等)一律不上界面。
- 字体改为系统字体栈(不再加载 Google Fonts),等宽只给 IP / 端口 / id / 命令;字号收成 13 / 14 / 15 三档。侧栏由 `js/app.js` 统一渲染,底部状态灯随 `/api/system` 真实同步。
- `adapters.yaml` 各类型 `desc` 改为面向用户的描述。
- 桌面版 noVNC 容器端口不再交给 Docker/lima publish；改由 app 为每条通道持有独立的 loopback SSH 转发，并由登录入口与看门狗按需自愈。
- 项目结构规整:`vpn-manager/` 拍平到仓库根;设计文档移入 `docs/`(`design.md`);README 改为中英双语(`README.md` / `README.zh-CN.md`)。
- 重新授权为 **MIT**(原为私有 / 保留所有权利);新增 `NOTICE` 第三方与闭源软件免责声明。
- 内部开发文档(架构 / 命门 / 贡献说明)集中到 `docs/development.md`。

### Added
- 独立开发 VM/数据/端口，开发模式禁止日常系统代理与助手写操作；`./verify.sh` 提供统一离线验证入口。
- 桌面出站诊断缓存：按 profile 定位 usernet、读取实际运行版本，展示有界 SYN_SENT 采样及最近守卫核对结果；来源不明时不归因到通道。
- 桌面版审计日志:每个改变状态的操作(通道增删改启停、规则增删改与批量启停、备注、上传、镜像源、配置导入导出、容器清理、入口接管、修复)记一条 `src=audit` 事件,`detail` 含目标与改前 / 改后快照(脱敏;密码、私钥、备注正文不入日志),供事后回滚判断。`GET|POST /api/events/enabled` 日志记录开关(持久化);`GET /api/events/export?from=&to=` 按日期区间导出。设置 → 运行日志新增「只看操作记录」、开关与日期导出。
- 桌面版新增通道「参与分流」、可逆的全局「全直连」、规则备注与批量保护，以及仅内存生效的自动修复暂停；三级开关统一折叠到同一有效规则集。
- 新增桌面控制端点:`GET|POST /api/routing`、`POST /api/system/self-heal`，并扩展通道与规则 PATCH / 配置导入导出字段。
- 配置导出导入:总览屏顶栏「导出 / 导入配置」;`GET /api/config/export` + `POST /api/config/import`(通道 + 分流规则 + 无头通道注入配置;交互登录密码不导出、导入后重新登录;导入只落库为「已停止」,按需逐个启动;Python 后端与 Rust core 双端等价)。
- 后端分流规则支持 `kind`(domain / ip)+ `enabled`,新增 IP-CIDR、rule-provider 与遥测。
- 共享 Web 页面接入真实 API，移除 mock `data.js`。
- Docker 栈冒烟脚本 `tests/smoke.sh`。
- 项目基础文件:README、LICENSE、CONTRIBUTING、.editorconfig、.dockerignore。

### Fixed
- noVNC 退出 lima gRPC 自动转发故障域；停止 / 删除与自动确保串行，避免 SSH 子进程在回收后竞态复活。
- arm64 上交互式 noVNC 登录 + mihomo 路由可用(websockify 自愈、路径尾斜杠)。
- 多处实机 UI 走查修正。

### Notes
- 历史从项目导入(落地方案 + FastAPI 后端 + mihomo)起算;此前未维护版本号。
